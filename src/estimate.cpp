              ...
              estimate_arguments const & estimate_args)
{
    ...
    auto clear_data = [&]()
    {
        ids.clear();
        seqs.clear();
        std::ranges::for_each(prev_counts, [](auto & v) { std::ranges::fill(v, float{}); });
        std::ranges::for_each(estimations, [](auto & v) { std::ranges::fill(v, uint16_t{}); });
    };
        ...
        for (size_t i = 0; i < seqs.size(); ++i)
        {
            ...
            for (size_t exp_idx = 0; exp_idx < num_experiments; ++exp_idx)
            {
                ...
                for (size_t l = 0; l < num_levels; ++l)
                {
                    ...
                    float const normalized = std::max(0.0f, static_cast<float>(corrected / (1.0 - fprs[level][exp_idx])));
                    ...
                    if (prev_count + normalized >= minimiser_pos)
                    {
                        ...
                        else
                        {
                            ...
                            if constexpr (samplewise)
                            {
                                ...
                                uint16_t const estimate = prev_threshold - static_cast<uint16_t>(normalized_minimiser_pos * threshold_diff);
                        ...
                        if constexpr (normalization_method && samplewise)
                            estimations[i][exp_idx] = static_cast<uint16_t>(estimations[i][exp_idx] / expressions[0][exp_idx]); // Normalize by first level
